...
export interface ProviderCampaignItemEn {
  id: string;
  name: string;
  description: string;
  purpose: string;
...
const initialProviderCampaignsEn: ProviderCampaignItemEn[] = [
  {
    id: 'camp-1',
    name: 'Purple Saturday Annual Initiative 2024',
    description: 'Participating as a strategic partner in the kingdom’s largest national initiative empowering persons with disabilities through exclusive 50% device discounts and fiber plan subsidies.',
    purpose: 'Enhance quality of life, expand digital inclusion, and ease household costs with the latest accessible assistive technologies.',
    targetAudience: 'Mu\'ini cardholders, traffic mobility permit holders, individuals with disabilities, and their caregivers',
    startDate: '2024-07-01',
    endDate: '2024-07-31',
    location: 'All certified branches across Saudi Arabia & mystc digital store',
    status: 'active',
  ...
  {
    id: 'camp-2',
    name: 'Saudi National Day 94 Accessibility Campaign',
    description: 'Launching a comprehensive suite of on-site accessibility services and distributing 1,000 smart accessible tablets to distinguished students with disabilities.',
    purpose: 'Celebrate national unity by reinforcing community solidarity and empowering students at the start of the academic year.',
    targetAudience: 'University students and vocational trainees with motor, visual, or hearing impairments',
    startDate: '2024-09-20',
    endDate: '2024-09-26',
    location: 'Riyadh, Jeddah, Dammam & National Digital Portal',
    status: 'upcoming',
  ...
  {
    id: 'camp-3',
    name: 'Universal Workplace Inclusion & Job Matching Forum',
    description: 'Interactive forum connecting talented national cadres with disabilities to accessible, fully equipped corporate and technical roles.',
    purpose: 'Achieve sustainable employment and fulfill Saudi Universal Building Code compliance across private sector workplaces.',
    targetAudience: 'Job seekers, career shifters, and recent graduates with disabilities',
    startDate: '2024-05-10',
    endDate: '2024-05-14',
    location: 'King Abdullah Financial District (KAFD) - Riyadh & Virtual Stream',
    status: 'completed',
...
interface ProviderCampaignsViewEnProps {
  onNavigate?: (tab: string) => void;
}

export const ProviderCampaignsViewEn: React.FC<ProviderCampaignsViewEnProps> = ({ onNavigate }) => {
  const [campaigns, setCampaigns] = useState<ProviderCampaignItemEn[]>(initialProviderCampaignsEn);
  const [selectedCampaign, setSelectedCampaign] = useState<ProviderCampaignItemEn | null>(null);
  ...
  const handleCreateCampaign = (e: React.FormEvent) => {
    ...
    const newCampaign: ProviderCampaignItemEn = {
      id: `camp-${Date.now()}`,
      name,
      description,
      purpose: purpose || 'Empowerment initiative supporting persons with disabilities and social inclusion.',
      targetAudience: targetAudience || 'All individuals with disabilities and their families',
      startDate: startDate || new Date().toISOString().split('T')[0],
      endDate: endDate || '2025-12-31',
      location: location || 'Branches & Online Portal',
      status: 'active',
      participantsCount: 0,
  ...
  return (
    <div id="mueeni-provider-campaigns-screen-en" className="space-y-6 pb-20 animate-fade-in text-left" dir="ltr">
      {/* Header Banner */}
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-bold px-3 py-1 rounded-full bg-purple-500/30 text-purple-200 border border-purple-400/30 flex items-center gap-1.5">
                ...
                <span>Campaigns & National Initiatives</span>
              </span>
            </div>
            <h1 className="text-xl sm:text-2xl font-black text-white">
              Provider Campaigns & Initiatives
            </h1>
            <p className="text-xs text-purple-100 mt-1 max-w-lg leading-relaxed">
              Launch and sponsor community empowerment campaigns and monitor engagement during national events
            </p>
          </div>

          <button
            id="btn-create-campaign-modal-en"
      ...
      <section className="grid grid-cols-3 gap-3">
        <div className="p-4 rounded-3xl bg-white border border-slate-200/90 shadow-2xs text-center">
          <span className="text-[11px] font-bold text-slate-500 block">Active Campaigns</span>
          <span className="text-2xl font-black text-purple-900 font-mono mt-1 block">
            {campaigns.filter((c) => c.status === 'active').length}
          </span>
          <span className="text-[10px] text-purple-600 font-bold mt-0.5 block">Accepting Beneficiaries</span>
        </div>
        ...
        <div className="p-4 rounded-3xl bg-white border border-slate-200/90 shadow-2xs text-center">
          ...
          <span className="text-[10px] text-emerald-600 font-bold mt-0.5 block">Via Digital Cards</span>
        </div>
        ...
        <div className="p-4 rounded-3xl bg-white border border-slate-200/90 shadow-2xs text-center">
          ...
          <span className="text-[10px] text-slate-500 font-bold mt-0.5 block">Verified Initiatives</span>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between px-1">
          <h2 className="text-base font-extrabold text-slate-900">
            Campaigns & Initiatives Directory
          </h2>
            ...
            >
              ...
              <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1 text-xs">
                <div className="flex items-start gap-1.5">
                  ...
                  <div>
                    <strong className="text-slate-800">Campaign Purpose: </strong>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-2 border-t border-slate-100 text-xs text-slate-500">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    <strong>Duration: </strong> {camp.startDate} - {camp.endDate}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-1.5 text-purple-800 font-bold">
                  ...
                  <span>
                    <strong>Beneficiaries Reached: </strong> {camp.participantsCount}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                <span className="font-bold text-slate-500 block">Full Description</span>
              ...
              <div className="p-3 rounded-2xl bg-purple-50/70 border border-purple-200/80 space-y-2">
                <div>
                  <span className="font-bold text-purple-900 block">Purpose & Goals</span>
                  <p className="text-purple-950 mt-0.5 leading-relaxed">{selectedCampaign.purpose}</p>
                </div>
                <div>
                  <span className="font-bold text-purple-900 block">Target Audience</span>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200">
                  <span className="font-bold text-slate-500 block">Start Date</span>
                  <span className="font-bold text-slate-900">{selectedCampaign.startDate}</span>
                </div>
                <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200">
                  <span className="font-bold text-slate-500 block">End Date</span>
                  <span className="font-bold text-slate-900">{selectedCampaign.endDate}</span>
                </div>
              </div>

              <div>
                <span className="font-bold text-slate-500 block">Scope & Location</span>
                <p className="text-slate-700 mt-0.5">{selectedCampaign.location}</p>
              </div>

              <div className="p-3 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center justify-between">
                <div>
                  <span className="font-bold text-emerald-900 block">Total Beneficiaries Engaged</span>
                  <span className="text-xs text-emerald-700">Verified via digital portal</span>
                </div>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateCampaign} className="space-y-3 text-xs">
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Universal Accessibility Initiative..."
                  value={name}
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">Campaign Purpose</label>
                <input
                  type="text"
                  placeholder="Promote accessibility and offer exclusive community support..."
                  value={purpose}
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Individuals with disabilities and their families..."
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">Location / Online</label>
                <input
                  type="text"
                  placeholder="Main headquarters, branches, or online..."
                  value={location}
