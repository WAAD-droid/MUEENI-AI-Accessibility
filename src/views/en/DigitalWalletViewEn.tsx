...
interface DigitalWalletViewEnProps {
  documents: WalletDocument[];
  onAddDocument: (doc: WalletDocument) => void;
  embedded?: boolean;
}

export const DigitalWalletViewEn: React.FC<DigitalWalletViewEnProps> = ({
  documents,
  onAddDocument,
  embedded = false,
}) => {
  ...
  const [uploadedFileName, setUploadedFileName] = useState<string | null>('Medical_Rehab_Evaluation_2024.pdf');
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    const newDoc: WalletDocument = {
      ...
      fileUrl: '#',
      details: {
        'File Name': uploadedFileName || 'uploaded_doc.pdf',
        'File Size': uploadFileSize,
        'Source': 'Manually uploaded by beneficiary',
        'AI Guidance Access': uploadAllowAI ? 'Enabled (leveraged for recommendations)' : 'Disabled',
      },
    };
  ...
  return (
    <div id="mueeni-wallet-screen-en" className={`space-y-5 ${embedded ? '' : 'pb-28'} animate-fade-in text-left`}>
      {/* Header Banner */}
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-purple-100 mt-1 max-w-md leading-relaxed">
              Your official digital cards, verified permits, and personal medical records in one secure, accessible place.
            </p>
          </div>
      ...
      <div className="flex rounded-2xl bg-slate-200/80 p-1.5 border border-slate-300 shadow-2xs">
        ...
        >
          ...
          <span>Verified Cards & Passes ({verifiedDocs.length})</span>
        </button>

        <button
          id="wallet-tab-my-docs-btn-en"
        ...
        >
          ...
          <span>Personal Documents ({myDocs.length})</span>
        </button>
      </div>
          ...
          <div className="flex items-center justify-between px-1">
            <div>
              <h3 className="text-sm sm:text-base font-extrabold text-slate-900">
                Official Digitally Linked Passes
              </h3>
              <p className="text-[11px] text-slate-500">
                Verified and synchronized via Nafath and government entities
              </p>
            </div>
                  ...
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-3 text-[10px] text-slate-400 font-mono mt-0.5">
                        <span>No: {doc.code}</span>
                        {doc.expiryDate && <span>• Valid: {doc.expiryDate}</span>}
                      </div>
                    </div>
                  </div>
      ...
      {activeCategoryTab === 'my_documents' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <div>
              <h3 className="text-sm sm:text-base font-extrabold text-slate-900">
                Uploaded Records & Reports
              </h3>
              <p className="text-[11px] text-slate-500">
                Your medical files, certificates, and rehabilitation documents
              </p>
            </div>
            <button
                  ...
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-slate-500 truncate mt-0.5">
                        Issuer: Uploaded by user • Date: {doc.issueDate}
                      </p>
                      ...
                      <div className="mt-1 flex items-center gap-1.5">
                        {doc.allowAI ? (
                          <span className="text-[10px] font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-md flex items-center gap-1">
                            ...
                            <span>Available for AI Guidance</span>
                          </span>
                        ) : (
      ...
      <div className="pt-2">
        ...
        >
          ...
          <span>+ Add New Document or Medical Report</span>
        </button>
      </div>
        ...
        >
          <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
                  ...
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-200">
                      {selectedDoc.category === 'verified'
                        ? 'Official Verified Digital Document'
                        : 'Personal Beneficiary Record'}
                    </span>
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-center">
              {selectedDoc.category === 'verified' ? (
                <div className="inline-block p-4 rounded-3xl bg-slate-50 border-2 border-dashed border-emerald-500/50 shadow-inner">
                  ...
                  <span className="text-[10px] font-mono text-emerald-800 font-bold mt-2 block">
                    Verification Hash: {selectedDoc.qrData || selectedDoc.code}
                  </span>
                </div>
              ) : (
                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-center space-y-2">
                  ...
                  <p className="text-xs text-slate-500">
                    Encrypted digital file securely preserved in MUEENI Cloud
                  </p>
                </div>
              )}
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-left space-y-2 text-xs">
                <div className="flex justify-between py-1 border-b border-slate-200/60">
                  <span className="text-slate-500 font-medium">Issue / Upload Date:</span>
              ...
              <div className="flex items-center justify-center gap-1.5 text-xs font-bold text-emerald-800">
                ...
                <span>
                  {selectedDoc.category === 'verified'
                    ? 'Digitally verified & compliant with national accessibility standards'
                    : 'Encrypted & ready for use in empowerment services'}
                </span>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
              <button
                onClick={() => {
                  alert('Digital pass successfully downloaded.');
                }}
          ...
          <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden p-6 space-y-4 text-left">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div>
                ...
                <p className="text-[11px] text-slate-500">
                  Upload your medical reports or training certificates for quick access
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4 text-xs">
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-slate-300 rounded-2xl p-4 text-center bg-slate-50 hover:bg-emerald-50/50 hover:border-emerald-400 transition-colors cursor-pointer">
                  ...
                  <span className="font-bold text-slate-800 block text-xs">
                    {uploadedFileName || 'Click here to choose file from your device'}
                  </span>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Comprehensive Rehab Assessment 2024..."
                  value={uploadTitle}
              ...
              <div>
                ...
                >
                  <option value="medical_file">Medical or Rehab Assessment</option>
                  <option value="certificate">Training Certificate / Course</option>
                  <option value="report">Employment Contract / Work Document</option>
                </select>
              </div>
                ...
                <label className="flex items-start gap-2.5 cursor-pointer">
                  ...
                  <div>
                    <span className="font-bold text-slate-900 block text-xs">
                      Allow AI Assistant to analyze this document for guidance
                    </span>
                    <p className="text-[10.5px] text-slate-600 leading-relaxed mt-0.5">
                      Empowers the AI Advisor to tailor answers and suggest career or training opportunities aligned with your profile.
                    </p>
                  </div>
                </label>
              </div>
